...
/* void parse_req_buf(char buff[1024], request *req) {
    ...
    while (rest[0] != '\r' || rest[1] != '\n') {
        header_len++;
        void *new_header =
            realloc(headers, sizeof(struct Header *) * header_len);
        if (!new_header) {
            perror("Failed to realloc memory for the header");
            return;
        } else {
        ...
        size_t key_len = strcspn(rest, ":");
        char *key = calloc(sizeof(char), key_len);
        memcpy(key, rest, &rest[key_len] - rest);
        key[key_len] = '\0';
        rest += key_len + 1;
        while (isspace(*rest)) {
            rest++; // "Host": "http://example.com".
        }

        size_t value_len = strcspn(rest, CRLF);
        char *value = calloc(sizeof(char), value_len);
        memcpy(value, rest, &rest[value_len + 1] - rest);
        value[value_len] = '\0';
        rest += value_len + 1;

        headers[header_len] = malloc(sizeof(Header *));
        headers[header_len]->key = key;
        headers[header_len]->value = value;
    }
